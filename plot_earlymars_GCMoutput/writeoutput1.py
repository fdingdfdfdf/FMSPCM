...
# dirname = '5_sh1.5e6/day8244h00.'#'6_sh3e6/day17175h00.' #'5_sh1.5e6/day8244h00.' #'3_sh9e4/day2748h00.'
# filename = '../data/SH/'+dirname+'atmos_avg.nc'
dirname = '3_re9e4/day4122h00.'#'7_re9e6/day20610h00.' #'5_re1.5e6/day6870h00.' #'3_re9e4/day4122h00.'
filename = '../data/RE20SH/'+dirname+'atmos_avg.nc'
outputname = 'cold_20um.nc'
...
temp  = f.variables['temp'].data[0,:,:,:]
cloud = f.variables['cld_amt'].data[0,:,:,:] #pres, lat, lon
cond  = f.variables['total_cond'].data[0,:,:,:]
evap  = f.variables['cld_evap'].data[0,:,:,:]
...
f = netcdf.netcdf_file(outputname, 'w')
f.history = 'Created for a test'
f.createDimension('pressure', len(levelp))
f.createDimension('lat', len(lat))
pressure = f.createVariable('pressure', 'f', ('pressure',))
pressure[:] = levelp
pressure.units = 'Pa'

latt = f.createVariable('lat', 'f', ('lat',))
latt[:] = lat
latt.units = 'deg'

t1 = f.createVariable('temperature','f',('pressure','lat'))
y1out  = vertical_interp(temp, pres, levelp)
t1[:,:] = np.mean(y1out,axis=2)
t1.units= 'Kelvin'

cld1 = f.createVariable('cloud mass concentration','f',('pressure','lat'))
y1out  = vertical_interp(cloud, pres, levelp)
cld1[:,:] = np.mean(y1out,axis=2)
cld1.units= 'kg/kg'

con1 = f.createVariable('condensation rate from water vapor to cloud','f',('pressure','lat'))
y1out  = vertical_interp(cond, pres, levelp)
con1[:,:] = np.mean(y1out,axis=2)
con1.units= 'kg/kg/s'

evap1 = f.createVariable('Evaporation rate of cloud','f',('pressure','lat'))
y1out  = vertical_interp(evap, pres, levelp)
evap1[:,:] = np.mean(y1out,axis=2)
evap1.units= 'kg/kg/s'
