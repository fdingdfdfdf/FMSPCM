...
#     rain   = f.variables['total_rain'].data[0,:,:]
    ...
    rehum= f.variables['rehum'].data[0,:,:,:] #pres, lat, lon
...
#plot
    ...
    # y1out  = vertical_interp(cloud*pres/rco2/temp, pres, levelp)
    # y1out  = vertical_interp(cloud/sphum, pres, levelp)
    y1out  = vertical_interp(rehum, pres, levelp)
    y1out[y1out<2e-15] = 2e-15
    ax = axes.flat[i]
    # CS2 = ax.contourf(X,Y, np.mean(y1out,axis=2), locator=mpl.ticker.LogLocator(), 
    #     levels=np.logspace(-15,6,21+1), cmap=cmap  )
        # levels=np.logspace(-15,-4,22+1), cmap=cmap  )
    CS2 = ax.contourf(X,Y,np.mean(y1out,axis=2)*100,levels=np.linspace(0,100,20+1),cmap=cmap)
    ax.set_yscale('log')
    ax.set_ylim([1e5, 4e2])
    if (i==0):
