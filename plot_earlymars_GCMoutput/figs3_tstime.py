...
#plot
    ax = axes.flat[i]
    ax.plot(time-time[0], np.mean(ts,axis=-1),'k-',label=r'1bar,25$^\circ$OBL,10$\mu$m')
...
#plot
    ax = axes.flat[i]
    ax.plot(time-time[0], np.mean(ts,axis=-1),'r--',label=r'1bar,45$^\circ$OBL,10$\mu$m')
...
dirname  = ['1_re9e2/day6870h00.',
    ...
    '7_re9e6/day20610h00.']
for i in range(len(dirname)):
    filename = '../data/RE20SH/'+dirname[i]+'atmos_daily.nc'
    # print filename
    f = netcdf.netcdf_file(filename, 'r')
    lat = f.variables['grid_yt'].data
    lon = f.variables['grid_xt'].data
...
#plot
    ax = axes.flat[i]
    ax.plot(time-time[0], np.mean(ts,axis=-1),'g-.',label=r'1bar,25$^\circ$OBL,20$\mu$m')
...
#plot
    ax = axes.flat[i]
    ax.plot(time-time[0], np.mean(ts,axis=-1),'b:',label=r'0.5bar,25$^\circ$OBL,10$\mu$m')
    ax.set_ylabel(r'Zonal mean T$_s$ (K)')
...
axes.flat[1].legend(numpoints=1,frameon=False, fontsize=10,
    bbox_to_anchor=(1.05, -0., 0.5, 1.), loc='center',
