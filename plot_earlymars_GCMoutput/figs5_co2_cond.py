...
#     cld_mean[i] = area_mean(vertical_integration(cloud, phalf), lat)
    co2cond = temp*0.
    tco2 = -3167.8/(np.log(pres/100) -23.23)
    co2cond[temp<tco2] = 1.
    # co2cond = tco2
...
#plot
    ...
    y2out  = vertical_interp(co2cond, pres, levelp)
    # y1out[y1out<2e-15] = 2e-15
    # ax = axes.flat[i]
    ax = fig.add_subplot(1,3,i+1)
    ax.contourf(X,Y, np.mean(y2out,axis=2), alpha = 0.5, 
        levels=[0,0.5,1], colors=['w','0.5'])
    CS2 = ax.contour(X,Y, np.mean(y1out,axis=2), levels=np.linspace(120,280,16+1),
        #locator=mpl.ticker.LogLocator(), levels=np.logspace(-15,-4,22+1), 
        cmap=cmap  )
    ax.clabel(CS2, CS2.levels[::2], inline = True, 
        fmt='%i')
