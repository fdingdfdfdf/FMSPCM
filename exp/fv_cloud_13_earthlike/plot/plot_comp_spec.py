...
filename = '../day7602h00.spectrum.nc'
f     = netcdf_file(filename, 'r')
# hrlw  = f.variables['hrlw'].data[0,:,:,:]
# hrsw  = f.variables['hrsw'].data[0,:,:,:]
# hr    = hrlw[::-1,:,:] + hrsw[::-1,:,:]
...
axes.plot(nu_lw, np.mean(olrnu,axis=-1),'k-',lw=0.5,label='FMS-PCM')
axes.plot(wnum,  np.mean(olr_airs,axis=-1),'r-',lw=0.5,label='AIRS')
axes.set_xlabel('Wavenumber (cm)')
axes.set_ylabel(r'Spectral OLR (W m$^{-2}$ cm$^{-1}$)')
axes.grid()
axes.legend(loc=0)
