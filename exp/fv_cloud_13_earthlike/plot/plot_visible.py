...
solar = np.sum((solarnu_mean[1:]+solarnu_mean[:-1])/2. \
    *(nu_sw[1:]-nu_sw[:-1]))
idx = np.abs(nu_sw - 3.12e4).argmin()
solar_uv = np.sum((solarnu_mean[idx+1:]+solarnu_mean[idx:-1])/2. \
    *(nu_sw[idx+1:]-nu_sw[idx:-1]))
    
idx1 = np.abs(nu_sw - 2.5e4).argmin()
idx2 = np.abs(nu_sw - 3.12e4).argmin()
solar_uva = np.sum((solarnu_mean[idx1+1:idx2]+solarnu_mean[idx1:idx2-1])/2. \
    *(nu_sw[idx1+1:idx2]-nu_sw[idx1:idx2-1]))
    
idx1 = np.abs(nu_sw - 1.4e4).argmin()
idx2 = np.abs(nu_sw - 2.5e4).argmin()
solar_vis = np.sum((solarnu_mean[idx1+1:idx2]+solarnu_mean[idx1:idx2-1])/2. \
    *(nu_sw[idx1+1:idx2]-nu_sw[idx1:idx2-1]))
    
print solar, solar_uv/solar, solar_vis/solar, solar_uva/solar
...
ax.axvspan(nu_sw[idx1], nu_sw[idx2], color='red', alpha=0.5)
