...
rcld = np.linspace(1e-6, 50e-6, 100)

temp = 290.
ka = 2.43e-2 *1.8325/1.718 *\
    (296.16+120)/(temp+120) *\
    (temp/296.16)**1.5
psi = 2.26e-5 *(temp/273.16)**1.81
term1 = Lv**2 / (ka*Rv*temp**2)
term2 = Rv*temp / (esat(temp)*psi)
timecld = 1e3*rcld**2*(term1+term2)/3/0.01
# timedif = 1e3*rcld**2*(term2)/3/0.01

fig, ax = plt.subplots()
ax.plot(rcld*1e6, 400/timecld, 'r-',label='T=290K, p=1bar')
# ax.plot(rcld, 400/timedif, 'b-',label='Diff only')
...
psi = 2.26e-5 *(temp/273.16)**1.81
term1 = Lv**2 / (ka*Rv*temp**2)
term2 = Rv*temp / (esat(temp)*psi*10)
timecld = 1e3*rcld**2*(term1+term2)/3/0.01
ax.plot(rcld*1e6, 400/timecld, 'b-',label='T=220K, p=.1bar')
...
psi = 2.26e-5 *(temp/273.16)**1.81
term1 = Lv**2 / (ka*Rv*temp**2)
term2 = Rv*temp / (esat(temp)*psi*1)
timecld = 1e3*rcld**2*(term1+term2)/3/0.01
ax.plot(rcld*1e6, 400/timecld, 'b--',label='T=220K, p=1bar')

ax.set_title('dt=400s, RH=99%')
ax.hlines(1,1,50,color='0.5')
# ax.plot(temp, term2, 'b-',label='diffusivity at 1bar')

# term3 = Rv*temp / (esat(temp)*psi*10)
# ax.plot(temp, term3, 'b--',label='diffusivity at .1bar')
# ax.fill_between(temp, term2, term3,facecolor='b',alpha=0.5)

ax.set_xlabel('cloud radius (um)')
ax.set_ylabel(r'dM/M')
ax.set_yscale('log')
# ax.invert_yaxis()
ax.grid()
ax.legend(loc=0)
