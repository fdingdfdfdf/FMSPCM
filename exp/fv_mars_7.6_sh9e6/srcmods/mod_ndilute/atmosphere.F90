...
!      dt_vg(:,:,k) = -v_tmp(:,:,k) / 86400.
   ...
   endwhere
      ...
      dt_ug(:,:,1:5) = -u_tmp(:,:,1:5) / 86400.
      dt_vg(:,:,1:5) = -v_tmp(:,:,1:5) / 86400.
...
!evaporation, convection
    q1 = qg_tmp(:,:, n)
    p1 = p_full(:, :, n)
    dp1 = p_half(:, :, n+1) - p_half(:, :, n)
    dmass = (p_half(:, :, n+1) - p_half(:, :, n)) * (1 - q1) /grav
    lmass = flux_q * delta_t
    rl = lmass / dmass
    rt = rl + q1 / (1. -q1)
    ct = cp_air + cp_water * rt
    ...
    qg_tmp(:,:,n) = (qg_tmp(:,:,n) + &
                       ...
                       flux_q * delta_t * grav &
                       / (p_half(:,:,n+1) - p_half(:,:,n)))
    p_half(:,:,n+1) = p_half(:,:,n+1) + flux_q * delta_t * grav                    
    p_full(:,:,n) = p_half(:,:,n) + (p_full(:,:,n) - p_half(:,:,n)) * &
                 (1. - q1) / (1. - qg_tmp(:,:,n))

    kappa1 =  (rdgas * (1.-q1) + rvgas*q1) &
             / (cp_air*(1.-q1) + cp_vapor*q1)
    tg_tmp(:,:,n) = tg_tmp(:,:,n) * (p_full(:,:,n) / p1) ** kappa1

    t11 = tg_tmp(:,:,n)    
    zh = rdgas* tg_tmp(:,:,n) / (1. + rt)* &
         (1. + (rt - rl)/d622)* &
         (log(p_half(:,:,n+1) / p_half(:,:,n)))
    Ee = flux_q * (zh)/2. + cp_water * (tg_tmp(:,:,n)) * flux_q

!------------------------------------------------------------------------------
!    t_surf = t_surf + (surf_lw_down + net_surf_sw_down-flux_r-flux_t) &
!               *delta_t / eff_heat_capacity
!$omp parallel do private(ij,i,j,k,m,is,ie,p_full,p_half)
do ij=1,tsiz
   j  = beglat + (ij-1) / nx
   is = 1 + isiz * mod(ij-1, nx)
...
! moist convection------------------------------------------------------
    ...
    call moist_convection(t_prev, q_prev, pfull_prev, phalf_prev, &
            rain1, t_after, q_after, pfull_after, phalf_after, Ep, &
rain_profile) 

    Ep = Ep / delta_t
    rain1 = rain1 / delta_t
    rain_profile = rain_profile / delta_t

!    call compute_k(t_after, q_after, phalf_after, k3)
!    rain(i,j)  = rain1
    conv_rain_profile(i,j,:) = rain_profile
    conv_rain(i,j)  = rain1

    !large-scale condensation
    !prev is actually after
    call lscale_cond(t_after, q_after, pfull_after, phalf_after, &
            rain1, t_prev, q_prev, pfull_prev, phalf_prev, Ep_2, &
rain_profile) 
    ...
    call compute_k(t_prev, q_prev, phalf_prev, k3)
!    rain(i,j)  = rain(i,j) + rain1
    cond_rain_profile(i,j,:) = rain_profile
    cond_rain(i,j)  = rain1  
    
    Emass = cp_water *t_surf(i,j) *(cond_rain(i,j) + conv_rain(i,j)-flux_q(i,j))
    ! update surface temperature and pressure
    call latent_heat(t11(i,j), lh)
    delta_t_surf = (surf_lw_down(i,j) + net_surf_sw_down(i,j)  &
                     - flux_t(i,j) - flux_r(i,j) - flux_q(i,j) * lh &
                     + Ep - Ee(i,j)-Emass) &
                    * delta_t / rho_cp / mld !eff_heat_capacity

    t_surf(i,j) = t_surf(i,j) + delta_t_surf
    psg_tmp(i,j) = psg_tmp(i,j) + &
                (flux_q(i,j) - conv_rain(i,j)-cond_rain(i,j))* grav *delta_t
    call get_eta_level(nlev, psg_tmp(i,j), pfull_after, phalf_after)
    lnp_full = log(pfull_after)
!    psg_tmp(i,j) = psg_tmp(i,j) + (flux_q(i,j) - rain(i,j))* grav *delta_t
!    call pressure_variables(phalf_after, lnp_half, pfull_after, lnp_full, &
!                            psg_tmp(i,j))
!    if ((i == 65) .and. (j==1)) print *, i,j,psg_tmp(i,j), t_after,q_after
! vertical interpolation
    water = q_prev(1:n) * (phalf_prev(2:n+1) - phalf_prev(1:n)) / grav
    water_1 = sum(water)

    lnpfull_prev = log(pfull_prev)

    call interp(n, lnpfull_prev, t_prev, &
               n, lnp_full, t_after)
    call interp(n, lnpfull_prev, q_prev, &
               n, lnp_full, q_after)
    call interp(n, lnpfull_prev, u_tmp(i,j,:), &
               n, lnp_full, u_after)
    call interp(n, lnpfull_prev, v_tmp(i,j,:), &
               n, lnp_full, v_after)

    do k = 1, n
       if (q_after(k) .lt. 0.) q_after(k) = 1.e-20
       
       !if (t_prev(k) .lt. 100) t_prev(k) = 100.
    end do 
   
    water = q_after(1:n) * (phalf_after(2:n+1) - phalf_after(1:n)) / grav
    water_2 = sum(water)

    q_after(n) = q_after(n) + (water_1 - water_2) * grav &
                       / (phalf_after(n+1) - phalf_after(n))
    
    call compute_k(t_after, q_after, phalf_after, k4)

    !correct the energy imbalance due to vertical interpolation
    t_surf(i,j) = t_surf(i,j) + (k3 - k4) /  rho_cp / mld !eff_heat_capacity
...
pme(:,:) = conv_rain(:,:) + cond_rain(:,:) - flux_q(:,:) 
call fv_array_sync() !probably needed before gmean
!ftop = gmean(nlon, mlat, beglat, endlat, net_surf_sw_down(1,beglat))
!if(master) write(6,*) 'mean incident flux=', ftop
ftop = gmean(nlon, mlat, beglat, endlat, pme(1,beglat))
!if(master) write(6,*) 'mean P-E (kg/m**2/s)=', ftop

mld = mld + ftop * delta_t / rho0
if (mld < 0.1) then
   mld = mld + 1.
   if(master) write(6,*) 'add another 1m of water into the slab ocean'
endif

pme(:,:) = t_surf(:,:) * (pme(:,:) - ftop) 
call fv_array_sync() !probably needed before gmean
ftop = gmean(nlon, mlat, beglat, endlat, pme(1,beglat))
delta_t_surf = ftop * delta_t / rho0 /mld
