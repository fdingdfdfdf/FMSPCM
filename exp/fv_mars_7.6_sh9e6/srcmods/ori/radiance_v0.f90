...
subroutine radiance_update (is, ie, js, je, num_levels, &
        Time_diag, agrid, q, q_cld, pt, ts, phalf, pfull, dTrad, dTs_rad )
...
!    real(8) lam_sw(nS)  ! wavelength [um]
  ...
  subroutine calculate_shortwave(iT1,cosa_i,T_lin_weight,ts,dp,f_i,f_cld,mu_avg,kappa_gray, &
...
!no shortwave abs
       ...
       do iLay = 1, nLay
          ...
          dtau_sw_ray(i,j,iLay,:) = 0.3066*2.49e-6 *dp(i,j,iLay)/grav *&
                (1e4/nu_sw(:))**(-4)
